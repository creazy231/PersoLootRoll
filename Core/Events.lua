...
function Self.RegisterEvents()
    ...
    Self:RegisterEvent("LEGACY_LOOT_RULES_CHANGED", "GROUP_CHANGED")
    Self:RegisterEvent("PLAYER_ENTERING_WORLD", "ZONE_CHANGED")
    Self:RegisterEvent("ZONE_CHANGED_NEW_AREA", "ZONE_CHANGED")
    Self:RegisterEvent("WALK_IN_DATA_UPDATE", "ZONE_CHANGED")
    -- Chat
...
function Self.UnregisterEvents()
    ...
    Self:UnregisterEvent("LEGACY_LOOT_RULES_CHANGED")
        ...
        if unit and result and from and to then
            ...
            if not UnitExists(unit) then
                for i=1,GetNumGroupMembers() do
                    local unitGroup = UnitName(IsInRaid() and "raid"..i or "party"..i)
