...
function Self.UnitAllow(unit)
    ...
    elseif config.allow.raidleader or config.allow.raidassistant then
        for i=1,GetNumGroupMembers() do
            local unitToken = IsInRaid() and "raid"..i or "party"..i
            if UnitIsUnit(unitToken, unit) then
                return config.allow.raidleader and UnitIsGroupLeader(unitToken)
                    or config.allow.raidassistant and UnitIsGroupAssistant(unitToken)
            end
        end
    end
...
function Self.SetRules(rules, silent)
    if Self.IsMasterlooter() then
        ...
        for i=1,GetNumGroupMembers() do
            local unitToken = IsInRaid() and "raid"..i or "party"..i
            local unit = UnitName(unitToken)
            if unit and not Unit.IsSelf(unit) then
                local rank = UnitIsGroupLeader(unitToken) and 2 or UnitIsGroupAssistant(unitToken) and 1 or 0
                if Self.IsOnCouncil(unit, true, rank) then
