...
function Self.IsGuildGroup(guild)
    ...
    for i=1,n do
        local g = Unit.GuildName(IsInRaid() and "raid"..i or "party"..i)
        if g then
...
function Self.IsCommunityGroup(commId)
    ...
    for i=1,n do
        local c = Unit.CommonClubs(IsInRaid() and "raid"..i or "party"..i)
        if c then
...
function Self.GetNumDroppedItems()
    ...
    if difficulty == DifficultyUtil.ID.DungeonChallenge then
        -- Midnight M+: always 2 items (no timing bonus or level scaling)
        return 2
    else
